<?php	session_start();	include ("connettiDb.php");
    ...
    <div id="menu">				<ul>					<li class="current_page_item"><a href="home.php">Home</a></li>					<li><a href="logout.php">logout</a></li>				</ul>   			</div>			   </div><!-- end #header -->
			   <div id="page" class="container">				<div id="marketing">					<p class="text1">Visualizza Dipendenti</p>				</div>				<div id="content">				<?php 
               	...
               	$border="1";
				 		...
				 		$corpo=tr($indici);						while($row=  mysql_fetch_array($query,MYSQL_BOTH))				 		{
				 										$riga_dati=td($row['cod_fiscale']).td($row['cognome']).td($row['nome']).td($row['indirizzo']);
							$riga_dati.=td($row['orario']);							
							$tip=$row['categoria'];							$queri = mysql_query("SELECT * FROM Categoria_Personale WHERE id_cat_per='$tip'");							$data=  mysql_fetch_array($queri);
							$riga_dati.=td($data['tipologia']);								                   if($_SESSION['tipo']=='direttore')                     {
                     	  	...
                     	  	$azioni.="<br>";   	                    	$azioni.="<input type=button value=\"Modifica\" onclick=\"location.href='modDipendenti.php?action=modvis&id=".$row['cod_fiscale']."'\"</center>";
   	                    	$riga_dati.=td($azioni);                     }          				$corpo.=tr($riga_dati);						}
